...
export const webhook: APIGatewayProxyHandler = async (event, _context) => {
    ...
    if (body) {
        if ('challenge' in body) {
            ...
            await pusher.trigger(`twitchEventSub_${body.event.broadcaster_user_id}`, 'subscriptionAdded', body.event)
        }
        if ('event' in body) {
            response = {
                statusCode: 200,
            ...
            await pusher.trigger(`twitchEventSub_${body.event.broadcaster_user_id}`, 'rewardClaimed', body.event)
        } else
            response = {
                statusCode: 418,
                body: JSON.stringify({
